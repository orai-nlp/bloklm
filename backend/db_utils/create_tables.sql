...
CREATE TABLE IF NOT EXISTS Fitxategia (
    ...
    text TEXT,
    charNum INTEGER,
    format VARCHAR(10) CHECK (format IN ('PDF', 'TXT', 'DOC', 'DOCX', 'SRT', 'MP3', 'WAV')),
...
CREATE TABLE IF NOT EXISTS Note (
    ...
    name VARCHAR(255) NOT NULL,
    type VARCHAR(10) CHECK (type IN ('summary', 'outline', 'glossary', 'FAQ', 'timeline', 'mindmap', 'podcast')),
    content TEXT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    contained_file_ids BIGINT[] DEFAULT '{}',
    bilduma_key BIGINT REFERENCES Bilduma(id) ON DELETE CASCADE
);
